...
<c:if test="${fn:length(rooms[version])!=0}">
	...
	<c:forEach items="${rooms[version]}" var="room" begin="0" step="1"
		varStatus="status">
		<div class="row" style="margin-bottom: 30px;">
			...
			<div class="span7">
				<h4>
					<c:choose>
						<c:when test="${fn:length(room.name) > 50}">
							<c:out value="${fn:substring(room.name, 0, 50)}..." />
						</c:when>
						<c:otherwise>
				...
				<blockquote>
					<small> <date:date pattern="HH时mm分 "
							value="${room.createAt}"></date:date> 有玩家 [<span
						style="color: #4B0082">${room_count[room.id]}</span>] 人
					</small> <small> <c:if test="${my_room.id==room.id}">你在这里</c:if> <c:if
							test="${room.status==0}">游戏中</c:if> <c:if
							test="${room.status==1}">未开始</c:if>
					</small> <small> 房主：<a href="/player/detail?uid=${room.createrID}"
