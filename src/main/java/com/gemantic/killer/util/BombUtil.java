...
public class BombUtil {
	...
	public static void getRoundPanes2(Pair pair, int rowCount, int columnCount,
			String system, Map<Pair, String> pairMap) {

		String systemValue = BombUtil.clickOpen(pair, system, rowCount,
				columnCount);
		if (BombUtil.Bomb.equals(systemValue)) {
			...
			if ("0".equals(systemValue)) {
				if (pairMap.containsKey(pair)) {
					...
					// 继续迭代
					pairMap.put(pair, systemValue);
					List<Pair> pairs = BombUtil.getRoundPanes(pair, rowCount,
							columnCount);

					for (Pair p : pairs) {
						BombUtil.getRoundPanes2(p, rowCount, columnCount,
								system, pairMap);
					}
				}
	...
	public static List<Pair> getRoundPanes(Pair pair, int rowCount,
			int columnCount) {
	...
	public static String clickOpen(Pair pair, String systemBombPic, int row,
			int column) {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * String bomb=BombUtil.initBomb(9,9); log.info(bomb); int
		 * index=BombUtil.convertIndex(3, 3, 9); String
		 * s=BombUtil.replaceBomb(index, bomb, "c"); log.info(s);
		 * 
		 * List<Pair>
		 * ls=BombUtil.getRoundPanes(BombUtil.convertString2Pair("1-1"), 9,
		 * 100);
		 * 
		 * log.info(ls);
		 */
		...
		Long start = System.currentTimeMillis();
		BombUtil.getRoundPanes2(BombUtil.convertString2Pair("16-18"), 20, 20,
				mineStr, pairs);
	...
	public static List<String> printPic(String systemBombPic, int row,
			int column) {
	...
	public static String convertPair2String(Pair pair) {
		...
		return String.valueOf(pair.getLeft() + BombUtil.Split_Underline
				+ pair.getRight());
	}
	...
	public static String generateSystemContent(Integer row, Integer column,
			String c) {
		List<Pair> pairs = new ArrayList();
		List<Pair> inits = new ArrayList();
		Map<Pair, String> pair_tags = new HashMap();
	...
	public static Integer countMine(String systemContent) {
		char[] cs = systemContent.toCharArray();
		int mine = 0;
		for (char c : cs) {
			if (c == '*') {
				mine++;
	...
	public static String generateUserContent(String userContent,
			String systemContent) {		
		StringBuffer sb=new StringBuffer();
		char[] cs = userContent.toCharArray();
		int index=0;
		for (char c : cs) {
			char v='n';
			if (c == 'n') {
